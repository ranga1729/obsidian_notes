...
## 6. Dedicated EC2
An EC2 instance run on hardware dedicated to a single customers. 
Use case : Workloads requiring hardware isolation, but not the full host control.
Pros : Hardware isolation.
Cons : Still managed by AWS and no transparency over inner workings. 