Worked on the back end modifications of the select order history. Fixed some issues there. Now its fine.

then I moved in to find a solution for the async function issue in the order-form task, it's getting worse. Found a new possible solution, but it affects the components modularity in a very bad way. 
Now I'm trying a different approach by publishing different events to do the scroll up and validation triggering, instead of using a one event for both functionalities.

I will have a huddle with Nuwan today to review the end point of the select order history and will discuss about the order form issue as well.

****
check the validation channels path to spot exactly where is the delay is happening.
Scenario: fill all the items before the heigh, weight(scroll up is not working correctly in this scenario.)

check if we can remove the setTimeOut.

apply those new changes to the other pages as well.

******
delivery status and order status are two different things.
if an delivery has happened, thos data will be in the log. 
check if there's an record to the specific order an  day id.
That's the condition to decide whether we should get details from the log or the other table. 

Remove the token look up, wait for the chomal's change and use it for the token value. 
Only check for the credit values. 

Do all of them in the same query.
(including the log record check and the credit check)
...
namespace FFF.Services.Business.FoodBox
{
    public class FoodBoxOrderService : IFoodBoxOrderService
    {
        ...
        public async Task<FoodBoxOrderDetailsDto> GetDetailsByIdAsync(string id)
        {
            var order = await _dataContext.FoodBoxOrders
                ...
                .Select(o => new FoodBoxOrderDetailsDto
                {
                    ...
                    Discount = o.Discount,
                    Total = o.Total,
                    OrderPlacementDate = o.CreatedDateUtc.ToUniversalTime(),
            ...
            {
                foreach (FoodBoxDeliveryDayDetailsDto day in order.DeliveryDays)
                {
                    day.Items = (order.Status == OrderStatus.Completed) ? GetCompletedOrdersItemDetail(order.Id, day.DayId) : GetOngoingOrdersItemDetails(order.Id, day.DayId);
                }
                ...
                if (transactionLedgers.ContainsKey(LedgerCodes.CustomerCredit))
                {
                    order.Credits = Math.Abs(transactionLedgers[LedgerCodes.CustomerCredit]);
                }
                if (transactionLedgers.ContainsKey(LedgerCodes.ActualTokenUtilization))
                {
                    order.Tokens = Math.Abs(transactionLedgers[LedgerCodes.ActualTokenUtilization]);
                }
            }
        ...
        private List<FoodBoxDeliveryItemDetailsDto> GetOngoingOrdersItemDetails(string orderId, string dayId)
        {
            var Items = _dataContext.FoodBoxDeliveryDayItems
        ...
        private List<FoodBoxDeliveryItemDetailsDto> GetCompletedOrdersItemDetail(string orderId, string dayId)
        {
            var Items = _dataContext.FoodBoxDeliveryDayItemLogs
